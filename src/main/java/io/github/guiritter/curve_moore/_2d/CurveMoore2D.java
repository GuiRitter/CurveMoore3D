...
import static io.github.guiritter.curve_moore._2d.GrayCode.grayCode;
import static io.github.guiritter.curve_moore._2d.MaximaOutput.maximaOutput;
...
/**
 * <p><a href="https://en.wikipedia.org/wiki/Moore_curve">Space-filling Moore curve</a> in two dimensions.
 ...
 * <p>Traversal generates an ordered list of 2D points.
 * @author Guilherme Alan Ritter
 */
public class CurveMoore2D {
     ...
     */
    public final CurveMoore2D nextOrder() {
        long translation = 1l << order; // 2 ^ order
        return new CurveMoore2D(order + 1,
     ...
     */
    public static final CurveMoore2D orderZero() {
        return new CurveMoore2D(0l, Arrays.asList(new Point(0,0)));
    }

    private CurveMoore2D(Long order, List<Point> pointCurrentList) {
        this.order = order;
        this.pointCurrentList = pointCurrentList;
        this.pointNextList = pointCurrentList;
    }

    private CurveMoore2D(Long order, List<Point> pointCurrentList, List<Point> pointNextList) {
        this.order = order;
        this.pointCurrentList = pointCurrentList;
    ...
    public static void main(String[] args) {
        CurveMoore2D curve = CurveMoore2D.orderZero();
        curve = curve.nextOrder().nextOrder().nextOrder().nextOrder();
